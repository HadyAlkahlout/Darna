...
class MarketReportsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun getComplete(from: String, to: String, page: Int) {
        val response = repository.completeMarketOrder(lang, token, from, to, page)
    ...
    private suspend fun getCanceled(from: String, to: String, page: Int) {
        val response = repository.canceledMarketOrder(lang, token, from, to, page)
        if (response.isSuccessful){
    ...
    private suspend fun getRefused(from: String, to: String, page: Int) {
        val response = repository.refusedMarketOrder(lang, token, from, to, page)
        if (response.isSuccessful){
