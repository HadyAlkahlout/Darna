...
class DeliveryOrdersViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val dataAssign = MutableLiveData<General>()
    ...
    private suspend fun orders(page: Int){
        val response = repository.getDeliveryOrders(lang,token,page)
    ...
    private suspend fun details(id: Int){
        val response = repository.deliveryOrderDetails(lang,token, id)
        if (response.isSuccessful){
    ...
    private suspend fun status(params: Map<String, RequestBody>){
        val response = repository.changeDeliveryOrderStatus(lang,token, params)
        if (response.isSuccessful){
            dataStatus.postValue(response.body())
        }
    }

    private suspend fun assign(params: Map<String, RequestBody>){
        val response = repository.assignDriver(lang,token, params)
        if (response.isSuccessful){
            dataAssign.postValue(response.body())
        }
    }
    ...
    fun assignDriver(params: Map<String, RequestBody>) {
        viewModelScope.launch {
            assign(params)
        }
    }
