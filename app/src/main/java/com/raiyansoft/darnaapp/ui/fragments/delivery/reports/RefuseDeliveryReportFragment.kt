...
import com.raiyansoft.darnaapp.databinding.FragmentRefuseDeliveryReportBinding
import com.raiyansoft.darnaapp.ui.viewmodel.DeliveryReportsViewModel
import com.raiyansoft.darnaapp.uitl.OnScrollListener

class RefuseDeliveryReportFragment : Fragment(), DatePickerDialog.OnDateSetListener {

    private lateinit var binding: FragmentRefuseDeliveryReportBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentRefuseDeliveryReportBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun doInitialization() {
        ...
        binding.rcRefusedReports.setHasFixedSize(false)
        binding.rcRefusedReports.adapter = adapter
        binding.rcRefusedReports.layoutManager = LinearLayoutManager(requireContext())
        binding.rcRefusedReports.addOnScrollListener(onScrollListener)
    ...
    private val onScrollListener = OnScrollListener {
        if (binding.rcRefusedReports.canScrollVertically(1)) {
    ...
    private fun getOrders(){
        loading = true
        toggleLoading()
        viewModel.refusedOrder(binding.tvFromDate.text.toString(), binding.tvToDate.text.toString(), currentPage)
        viewModel.dataRefused.observe(viewLifecycleOwner,
            {response->
                loading = false
                toggleLoading()
