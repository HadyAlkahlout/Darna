...
import com.raiyansoft.darnaapp.databinding.FragmentCancelDeliveryReportBinding
import com.raiyansoft.darnaapp.ui.viewmodel.DeliveryReportsViewModel
import com.raiyansoft.darnaapp.uitl.OnScrollListener

class CancelDeliveryReportFragment : Fragment(), DatePickerDialog.OnDateSetListener {

    private lateinit var binding: FragmentCancelDeliveryReportBinding
    private val dateDialog by lazy {
        com.raiyansoft.darnaapp.dialog.DatePickerDialog(this)
    }
    private var type = 0
    private val viewModel by lazy {
        ViewModelProvider(this)[DeliveryReportsViewModel::class.java]
    }
    ...
    override fun onCreateView(
        ...
        binding = FragmentCancelDeliveryReportBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun doInitialization() {
        ...
        binding.rcCanceledReports.setHasFixedSize(false)
        binding.rcCanceledReports.adapter = adapter
        binding.rcCanceledReports.layoutManager = LinearLayoutManager(requireContext())
        binding.rcCanceledReports.addOnScrollListener(onScrollListener)
    ...
    private val onScrollListener = OnScrollListener {
        if (binding.rcCanceledReports.canScrollVertically(1)) {
    ...
    private fun getOrders(){
        loading = true
        toggleLoading()
        viewModel.canceledOrder(binding.tvFromDate.text.toString(), binding.tvToDate.text.toString(), currentPage)
        viewModel.dataCanceled.observe(viewLifecycleOwner,
            {response->
                loading = false
                toggleLoading()
